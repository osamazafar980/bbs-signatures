...
import {CalculatePseudonym} from './pseudonym.js';
...
export async function ProofGenWithPseudonym({
  ...
  an additional generator, and the creation of `Pseudonym` from `verifier_id`
  and `pid` and passing it, `verifier_id`, and `pid_scalar` to
  `CoreProofGenWithPseudonym` instead of `CoreProofGen`. */
  ...
  */
  ...
  const Pseudonym = CalculatePseudonym({verifier_id, pid, api_id, ciphersuite});
