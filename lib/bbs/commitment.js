...
export function deserialize_and_validate_commit({
  ...
  */
  if(commitment_with_proof_octets.length === 0) {
    // Identity_G1 == ciphersuite.E1.ONE
    return [ciphersuite.E1.ONE, 0];
  }
  ...
  // scalars have a length of >= 0; and `length(commit_proof[1])` here refers
  // to the length of the middle scalars so we compute it by subtracting
  // the first and last (i.e., `commitment_proof.length - 2`)
  ...
  // number of blind_generators needs to be M + 1, which means slice(1, M + 2)
  const blind_generators = generators.slice(1, M + 2);
