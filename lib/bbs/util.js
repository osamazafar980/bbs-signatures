...
export function calculate_domain({
  ...
  */
  const domain_dst = concatBytes(api_id, TEXT_ENCODER.encode('H2S_'));
...
export function get_disclosed_data({
  ...
  */
  ...
  // combine the disclosed messages
  // FIXME: does the spec put these in the wrong order / inconsistent with
  // how `message_scalars` are set in `BlindProofGen`?
  const disclosed_messages = [
    ...messages,
    ...committed_messages
  ];

  return {disclosed_messages, disclosed_indexes: indexes};
}
